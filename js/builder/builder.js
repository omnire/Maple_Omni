...
window.initOmniBuilderTab = function() {
    ...
    if (!window.omniSimMemory.initialLoaded) {
        ...
        const savedPresetKey = `omni_builder_active_preset_${charName}`;
        const localPreset = localStorage.getItem(savedPresetKey);
        
        // 💡 [초보자 가이드] 이전에 사용자가 설정해둔 보스 세팅 프리셋 기억(캐시)이 있다면 우선적으로 불러오고, 없으면 전투력이 높은 최적의 보스 프리셋을 자동 탐색합니다.
        if (localPreset) {
            window.omniSimMemory.activePreset = parseInt(localPreset, 10);
        } else {
    ...
    rootBox.innerHTML = `
        <!-- 💡 [초보자 가이드 - 캐릭터 정보 카드] 그림자 효과를 완전히 제거하고 아바타를 왼쪽에 독립 배치, 나머지 정보와 안내 문구를 오른쪽에 정렬하여 깔끔하게 다듬었습니다 -->
        <div style="background: var(--builder-card-bg); border-radius: 16px; padding: 20px 24px; border: 1px solid var(--builder-border); display: flex; flex-direction: column; gap: 16px; width: 100%; box-sizing: border-box; margin-bottom: 24px;">
            <div style="display: flex; align-items: center; gap: 20px; width: 100%;">
                
                <!-- 🎨 아바타 이미지만 독립적으로 왼쪽에 배치 (카드 및 원형 그림자 제거) -->
                <div style="display: flex; align-items: center; flex-shrink: 0;">
                    <div class="builder-profile-avatar-circle" style="border: 2px solid var(--builder-border); width: 68px; height: 68px;">
                ...
                <!-- 📋 나머지 정보 및 스탯 지표를 오른쪽에 정렬 -->
                <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px; width: 100%; min-width: 0;">
                    <div>
                        <div style="font-size: 15px; font-weight: 900; color: var(--builder-text-main); display: flex; align-items: center; gap: 8px;">
                            <span>${window.currentSearchData.basic.character_name}</span>
                            <span style="font-size: 9.5px; background: var(--builder-bg-sub); color: var(--builder-text-accent); padding: 2px 8px; border-radius: 12px; font-weight: 800; border: 1px solid var(--builder-border);">LIVE SPEC PRESET</span>
                        </div>
                        <div style="font-size: 11.5px; color: var(--builder-text-sub); font-weight: 700; margin-top: 4px; display: flex; align-items: center; gap: 6px;">
                            <span style="color: var(--builder-text-main); font-weight: 800;">Lv.${window.currentSearchData.basic.character_level || 282}</span>
                            <span>·</span>
                            <span>${window.currentSearchData.basic.world_name || '엘리시움'}</span>
                            <span>·</span>
                            <span style="color: var(--builder-text-accent);">${window.currentSearchData.basic.character_class || '초보자'}</span>
                        </div>
                    </div>

                    <div style="display: flex; gap: 8px; align-items: center; background: var(--builder-bg-sub); padding: 6px 10px; border-radius: 12px; border: 1px solid var(--builder-border);">
                        <div style="background: var(--builder-card-bg); border: 1px solid var(--builder-border); padding: 6px 12px; border-radius: 8px; font-size: 11px; text-align: center; min-width: 65px;">
                            <div style="color: var(--builder-text-sub); font-size: 9px; font-weight: 800;">길드</div>
                            <div style="color: var(--builder-text-main); font-weight: 900; margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 80px;">${charGuild}</div>
                        </div>
                        <div style="background: var(--builder-card-bg); border: 1px solid var(--builder-border); padding: 6px 12px; border-radius: 8px; font-size: 11px; text-align: center; min-width: 65px;">
                        ...
                        <div style="background: var(--builder-card-bg); border: 1px solid var(--builder-border); padding: 6px 12px; border-radius: 8px; font-size: 11px; text-align: center; min-width: 65px;">
                            <div style="color: var(--builder-text-sub); font-size: 9px; font-weight: 800;">유니온</div>
                            <div style="color: var(--builder-text-main); font-weight: 900; margin-top: 2px;">${charUnion}</div>
                        </div>
                        <div style="background: var(--builder-card-bg); border: 1px solid var(--builder-border); padding: 6px 12px; border-radius: 8px; font-size: 11px; text-align: center; min-width: 65px;">
                            <div style="color: var(--builder-text-sub); font-size: 9px; font-weight: 800;">무릉기록</div>
            ...
            <!-- 💡 안내 문구 영역 -->
            <div style="background: var(--builder-bg-sub); border: 1px dashed var(--builder-border-dashed); border-radius: 10px; padding: 10px 16px; font-size: 11.5px; color: var(--builder-text-sub); display: flex; align-items: center; gap: 8px; width: 100%; box-sizing: border-box;">
                <span>💡</span>
                <span style="font-weight: 700;">이미 사냥 세팅일 경우에는 인게임에서 보스 세팅으로 변경 후 API 갱신을 진행해 주세요.</span>
            </div>
        </div>
...
window.switchBuilderPreset = function(presetNumber) {
    ...
    // 💡 [초보자 가이드] 프리셋을 변경할 때마다 해당 캐릭터의 선호 보스 세팅 번호를 로컬 스토리지에 영구 캐싱합니다.
    const charName = window.currentSearchData?.basic?.character_name || "default";
    localStorage.setItem(`omni_builder_active_preset_${charName}`, presetNumber);
