/**
 * ============================================================================
 * 🧠 MAPLE OMNI V14 - js/hunt/overview.js [통합 엔진 코어]
 * 설명: 메인 슬롯 라우팅 및 캘린더 엔진을 전담하는 중앙 분배 코어 모듈입니다.
 * 규칙: 초보자도 이해할 수 있도록 코드 전체에 상세한 한글 주석을 제공합니다.
 * ============================================================================
 */
...
window.openCharRegisterModal = function(slotId, event) {
    ...
    if (modalOverlay) {
        ...
        const inputEl = document.getElementById('v14_modal_search_name');
        if (inputEl) {
            inputEl.value = existingData.name || '';
...
window.deleteCharRegisterData = function(slotId, event) {
    if (event) event.stopPropagation();
    if (confirm(`⚠️ [슬롯 초기화] Slot-${slotId}번 캐릭터의 연동 자원을 영구 삭제할까요?`)) {
...
window.saveCharRegisterDataFromApi = async function() {
    ...
    const inputEl = document.getElementById('v14_modal_search_name');
    const searchName = inputEl ? inputEl.value.trim() : '';
    if (!searchName) { alert("조회할 캐릭터 닉네임을 정확히 입력하세요."); return; }

    const saveBtn = document.getElementById('v14ModalSaveBtn');
    if (saveBtn) { saveBtn.innerText = "⏳ API 동기화 중..."; saveBtn.disabled = true; }
    ...
    try {
        ...
        if (saveBtn) { saveBtn.innerText = "🔍 캐릭터 동기화 및 등록"; saveBtn.disabled = false; }
    }
};
...
window.renderHuntOverviewPage = function() {
    ...
    const innerTabBarHtml = `
        <div class="v14-inner-tab-bar">
            ...
            ${(window.overviewMainTab === 'hunt' && window.overviewSelectedCharId !== null) ? `
                <button type="button" class="v14-back-list-btn" onclick="window.resetCharOverviewToGrid()">
                    ← 캐릭터 선택 그리드 목록으로 이동
                </button>
            ` : ''}
        </div>
    ...
    if (window.overviewMainTab === 'hunt') {
        if (window.overviewSelectedCharId === null) {
            ...
            for (let slot = 1; slot <= 5; slot++) {
                ...
                const cName = charMeta.name || `슬롯 Slot-${slot}`;
                ...
                const avatarGraphic = cAvatar
                    ? `<img src="${cAvatar}" alt="avatar" class="v14-safe-avatar" onerror="window.handleAvatarLoadError(this);">
                       <span class="v14-fallback-maple" style="display:none; font-size:28px; color:var(--accent-purple);">🍁</span>`
                    : `<span style="font-size:26px; color:var(--text-muted);">➕</span>`;

                const deleteButtonMarkup = hasData
                    ? `<button type="button" class="v14-slot-delete-btn" onclick="window.deleteCharRegisterData(${slot}, event)" title="슬롯 삭제">❌ 삭제</button>`
                    : '';

                gridCardsHtml += `
                    <div class="v14-selector-card ${hasData ? 'has-character' : 'empty-slot'}" onclick="${hasData ? `window.selectCharOverviewDetail(${slot})` : `window.openCharRegisterModal(${slot}, event)`}">
                        <div class="v14-card-top-controls">
                            ${deleteButtonMarkup}
                            <button type="button" class="v14-slot-edit-btn" onclick="window.openCharRegisterModal(${slot}, event)">${hasData ? "🔄 갱신" : "📥 등록"}</button>
                        </div>
                        
                        <div class="v14-avatar-square-box">
                            ${avatarGraphic}
                        </div>
                        ...
                        <div class="v14-card-metric-divider">
                            <div class="v14-metric-row"><span>⏱️ 누적 사냥시간:</span><span class="v14-card-stat-val">${slotTimeStr}</span></div>
                            <div class="v14-metric-row"><span>💰 누적 획득메소:</span><span class="v14-card-stat-val">${slotMesoSum.toLocaleString()}</span></div>
                        </div>
                    </div>
                `;
            }
            ...
            container.innerHTML = `
                ...
                <div id="v14RegModalOverlay" class="v14-reg-modal-overlay" onclick="window.closeCharRegisterModal();">
                    <div class="v14-reg-modal-window" onclick="event.stopPropagation();">
                        <div class="v14-modal-header">
                            <span class="v14-modal-icon">🔍</span>
                            <div class="v14-modal-title-group">
                                <div class="v14-modal-title">캐릭터 닉네임 연동</div>
                                <div class="v14-modal-subtitle">등록할 메이플스토리 닉네임을 입력하세요.</div>
                            </div>
                        </div>
                        
                        <div class="v14-modal-body">
                            <input type="text" id="v14_modal_search_name" class="v14-modal-input" placeholder="닉네임 입력" onkeypress="if(event.key === 'Enter') window.saveCharRegisterDataFromApi();">
                            <div class="v14-modal-notice-lbl">
                                💡 넥슨 OpenAPI를 통해 아바타 이미지, 직업, 레벨이 실시간으로 연동됩니다.
                            </div>
                        </div>
                        ...
                        <div class="v14-modal-btn-row">
                            ...
                            <button type="button" id="v14ModalSaveBtn" class="v14-mbtn v14-mbtn-save" onclick="window.saveCharRegisterDataFromApi();">🔍 캐릭터 동기화 및 등록</button>
                        </div>
                    </div>
                </div>
            `;
        ...
        const finalAvatarMarkup = characterAvatarUrl
            ? `<img src="${characterAvatarUrl}" alt="Avatar" onerror="window.handleAvatarLoadError(this);"><span style="display:none; font-size:26px; color:var(--accent-purple);">🍁</span>`
            : `<span style="font-size:26px; color:var(--text-muted);">🍁</span>`;
        ...
        for (let i = 0; i < 7; i++) {
            ...
            /* 💡 [교정] 14 + 20 + 17 + 10 + 10 + 11 + 11 + 7 = 100% 비율 맞춤 */
            tableRowsHtml += `
                <tr class="v14-interactive-row">
                    <td class="v14-td-date" style="width: 14%; text-align: center;">
                        <div>${dateStr}</div>
                        <div class="v14-table-subtext">⏱️ ${timeString}</div>
                    </td>
                    <td style="width: 20%; text-align: left; padding-left: 8px;">
                        <span class="v14-map-badge">📍 ${uniqueMaps}</span>
                    </td>
                    <td class="v14-td-meso" style="width: 17%; text-align: right; padding-right: 10px;">
                        <div>+ ${dayMesoSum.toLocaleString()}</div>
                        <div class="v14-table-subtext-purple">📈 ${hourlyMesoForm}</div>
                    </td>
                    <td class="v14-td-gem" style="width: 10%; text-align: center;">💎 ${dayGemSum}개</td>
                    <td class="v14-td-frag" style="width: 10%; text-align: center;">✨ ${dayFragSum}개</td>
                    <td class="v14-td-subincome" style="width: 11%; text-align: right; padding-right: 8px;">+ ${daySubIncome.toLocaleString()}</td>
            ...
            tableRowsHtml = `
                <tr>
                    <td colspan="8" class="v14-empty-table-notice">
                        📡 최근 7일 동안 기록된 사냥 세션 데이터가 존재하지 않습니다.
                    </td>
                </tr>
            `;
        }
            ...
            <div class="v14-overview-wrapper">
                ...
                <div class="v14-top-flex-zone">
                    <div class="v14-char-profile-card">
                        <div class="v14-avatar-square-box-detail">${finalAvatarMarkup}</div>
                    ...
                    <div class="v14-timeline-data-card">
                        <div class="v14-section-title-bar">⚔️ 최근 7일 사냥 상세 내역</div>
                        <div style="max-height: 254px; overflow-y: auto; border-radius:8px; border:1px dashed var(--border-color);" class="v14-table-overflow-frame">
                            <table class="v14-timeline-table">
                                <thead>
                                    <tr>
                                        <!-- 💡 [교정] th 열 너비 합계 = exatamente 100% (14+20+17+10+10+11+11+7) -->
                                        <th style="width: 14%; text-align: center;">정산기준일</th>
                                        <th style="width: 20%; text-align: left; padding-left: 8px;">주요 활동 사냥터</th>
                                        <th style="width: 17%; text-align: right; padding-right: 10px;">획득 순메소액</th>
                                        <th style="width: 10%; text-align: center;">코어 젬스톤</th>
                                        <th style="width: 10%; text-align: center;">에르다 조각</th>
                                        <th style="width: 11%; text-align: right; padding-right: 8px;">부수입 순익</th>
                                        <th style="width: 11%; text-align: center;">성장 스케일</th>
                                        <th style="width: 7%; text-align: center;">관제</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ${tableRowsHtml}
                ...
                <div style="width: 100%;">
                    <div class="v14-section-title-bar">📋 통합 정산 요약 리포트</div>
                    <div class="v14-bottom-summary-grid">
                        <div class="v14-summary-bento-box"><div class="v14-bento-label">💰 7일 전체 메소 총액</div><div class="v14-bento-value total-meso-color">${accumulatedMeso7Days.toLocaleString()}</div></div>
                        <div class="v14-summary-bento-box"><div class="v14-bento-label">📈 하루 평균 총액 </div><div class="v14-bento-value total-avg-color">${averageDailyMeso.toLocaleString()}</div></div>
    ...
    if (window.overviewMainTab === 'integrated') {
        ...
        if (window.overviewIntegratedSubView === 'list') {
            bottomContentSegmentHtml = `
                <div class="v14-timeline-data-card" style="width:100%;">
                    <div class="v14-section-title-bar">🏆 전 캐릭터 사냥 종합 랭킹 리더보드</div>
                    <div class="v14-leaderboard-twin-grid">
                        ...
                        <div class="v14-leaderboard-panel-box">
                            <div class="v14-leaderboard-panel-title">⏱️ 누적 사냥 가동 시간 랭킹 (총 시간단위)</div>
                            ${grindLeaderboardHtml}
                        </div>
                    </div>
                </div>
        ...
        container.innerHTML = `
            <div class="v14-overview-wrapper">
                ...
                <div style="width: 100%;">
                    <div class="v14-section-title-bar-noborder">📊 전체 캐릭터 결산 지표 </div>
                ...
                <div style="display:flex; gap:6px; margin: 2px 0;">
                    <button type="button" class="v14-back-list-btn filter-toggle-btn" style="background:${window.overviewIntegratedSubView === 'list' ? 'var(--btn-active-bg)' : 'var(--card-bg)'};" onclick="window.switchIntegratedSubView('list');">🏆 랭킹 리더보드 그래프 보기</button>
                    <button type="button" class="v14-back-list-btn filter-toggle-btn" style="background:${window.overviewIntegratedSubView === 'calendar' ? 'var(--btn-active-bg)' : 'var(--card-bg)'};" onclick="window.switchIntegratedSubView('calendar');">📅 통합 출석 달력 보기</button>
                </div>
