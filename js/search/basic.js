...
window.generateAuxiliaryStatGrid = function(finalStat) {
    ...
    trackingTargets.forEach(stat => {
        ...
        layoutHtml += `
            <div class="stat-grid-item-row" style="display:flex; justify-content:space-between; align-items:center; background: var(--omni-card-bg, #f8fafc); border:1px solid var(--omni-card-border-line, #e2e8f0); padding:6px 12px; border-radius:8px; font-size: 11.5px;">
                <span style="color: var(--omni-text-muted, #475569); font-weight: 700;">${stat.title}</span>
                <span style="color: var(--omni-text-dark, #0f172a); font-weight: 800;">${val}</span>
            </div>
        `;
    });
