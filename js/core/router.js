...
document.addEventListener('DOMContentLoaded', () => {
    const activeKey = localStorage.getItem('nexon_api_key') || localStorage.getItem('omni_api_key');
    
    // 만약 API 키 정보가 로컬 스토리지에 아예 없다면 인트로 화면을 유지합니다.
    if (!activeKey) {
        return;
    }

    // 💡 [새로고침/코드저장 튕김 방지 가드] API 키가 존재할 경우 인트로 레이어를 무조건 차단하고 마지막 세션 페이지를 즉시 복원합니다.
    setTimeout(() => {
        sessionStorage.setItem('omni_session_active', 'true');
        
        // 인트로 오버레이 강제 은폐
        const introOverlays = document.querySelectorAll('#introScreen, .intro-screen, #omniIntroOverlay');
        introOverlays.forEach(overlay => {
            if (overlay) overlay.style.setProperty('display', 'none', 'important');
        });
        
        if (typeof window.hideIntro === 'function') {
            try { window.hideIntro(); } catch(e) {}
        }

        // 마지막 작업 중이던 페이지 ID 복원 (기본값: 대시보드)
