/**
 * ============================================================================
 * 📑 MAPLE OMNI V15 - js/todo/js/todo_planner.js [📅 PLANNER & HUNTING EXP SYNC]
...
function getTodayDateString() {
    ...
    return `${year}-${month}-${day}`; // 최종 형태: "2026-07-29"
}
...
/**
 * 💡 캐릭터 데이터 객체 및 OMNI V15 캐시 데이터베이스에서 Nexon Open API 공식 서버(월드) 명칭을 정밀 추출합니다.
 * 초보자 가이드: Nexon Open API의 world_name 키, OMNI V15 캐릭터 캐시, 스케줄러 리스트까지 다각도로 정밀 추적하여
 * "스카니아", "크로아", "엘리시움" 등의 순수 서버명만 정확하게 반환합니다.
 */
window.getMapleServerName = function(charObj) {
    ...
    if (typeof charObj === 'string') {
        ...
        cName = charObj.name || charObj.character_name || charObj.characterName || charObj.id || '';
    }

    // 1단계: charObj 자체 속성에 world_name이 존재하는지 체크
    let s = (typeof charObj === 'object' && charObj !== null) ? (charObj.world_name || charObj.worldName || charObj.world || charObj.server || charObj.serverName || charObj.character_world || charObj.world_id || '') : '';

    // 2단계: OMNI V15/V14 개별 캐릭터 조회 캐시(omni_v15_cached_char_닉네임)에서 정밀 교차 검증
    if (!s && cName) {
        try {
            const raw15 = localStorage.getItem(`omni_v15_cached_char_${cName}`);
            const raw14 = localStorage.getItem(`omni_v14_cached_char_${cName}`);
            const targetRaw = raw15 || raw14;
            if (targetRaw) {
                const parsed = JSON.parse(targetRaw);
                if (parsed && parsed.basic) {
                    s = parsed.basic.world_name || parsed.basic.worldName || '';
                } else if (parsed) {
                    s = parsed.world_name || parsed.worldName || parsed.world || '';
                }
            }
        } catch(e) {}
    }

    // 3단계: 스케줄러 명단 리스트(omni_v14_todo_characters_list) 스토리지 재검색
    if (!s && cName) {
        try {
            const rawList = localStorage.getItem("omni_v14_todo_characters_list");
            if (rawList) {
                const list = JSON.parse(rawList);
                if (Array.isArray(list)) {
                    const found = list.find(item => (item.name || item.character_name || item.characterName || item.id) === cName);
                    if (found) {
    ...
    // 4단계: 최근 활성 검색 데이터(omni_last_active_search_data)에서 검색
    if (!s && cName) {
        try {
            const rawLast = localStorage.getItem("omni_last_active_search_data");
            if (rawLast) {
                const parsedLast = JSON.parse(rawLast);
                if (parsedLast && parsedLast.basic && (parsedLast.basic.character_name === cName || parsedLast.basic.name === cName)) {
                    s = parsedLast.basic.world_name || '';
                }
            }
        } catch(e) {}
    }
...
window.calculateExpPrediction = function(charName, targetLevel) {
    const char = window.omniTodoState.characters.find(c => (c.name || c.id) === charName);
    const currentLevel = char ? parseInt(char.level || char.character_level || 260, 10) : 260;
    ...
    let huntingLogs = [];
    try {
        const rawLogs = localStorage.getItem("omni_v14_hunting_integrated_log") || localStorage.getItem("omni_v14_hunting_logs") || localStorage.getItem("omni_hunting_log");
        if (rawLogs) huntingLogs = JSON.parse(rawLogs);
    } catch(e) { huntingLogs = []; }

    let charExpAmounts = [];
    if (Array.isArray(huntingLogs)) {
        huntingLogs.forEach(log => {
            const logChar = log.character || log.charName || log.characterName || log.name || "";
            if (logChar === charName) {
                const expVal = Number(log.expGain || log.exp || log.experience || log.gainExp || 0);
                if (expVal > 0) {
...
window.renderExpChart = function(charName) {
    ...
    if (charName === '전체') {
        ...
        if (wrapper) {
            wrapper.innerHTML = `
                <div id="omniExpGuideBox" style="padding: 24px; text-align: left; color: var(--omni-text-sub, #64748b); font-size: 12.5px; line-height: 1.7; display: flex; flex-direction: column; gap: 10px; height: 100%; box-sizing: border-box; justify-content: center; background: var(--omni-bg-clean, #f8fafc); border-radius: 12px; border: 1px dashed var(--omni-card-border-line, #cbd5e1);">
                    <div>• 상단 좌측의 <strong>캐릭터 선택 셀렉트박스</strong>에서 원하시는 캐릭터를 선택하시면, 해당 캐릭터의 최근 30일간 일자별 레벨업 추이 그래프와 사냥기록지 연동 리포트가 즉시 활성화됩니다.</div>
                    <div>• 본 시스템의 성장 분석 및 목표 도달 기일은 <strong>[사냥/재획] 사냥기록지</strong>에 기록된 실제 사냥 지표와 완벽하게 결합되어 추산됩니다.</div>
                    <div style="font-size: 11.5px; color: #94a3b8; border-top: 1px dashed #cbd5e1; padding-top: 8px; margin-top: 2px;">
                        🔮 우측 성장 스케줄러 달력의 날짜를 클릭하시면, 해당 일자의 시간대별 경험치 획득량 상세 추이를 확인하실 수 있습니다.
                    </div>
                </div>
            `;
        }
    ...
    const charObj = window.omniTodoState.characters.find(c => (c.name || c.id) === charName);
    const curLv = charObj ? parseInt(charObj.level || charObj.character_level || 283, 10) : 283;
...
window.renderTodoPlannerContent = function() {
        ...
        if (currentFilter === '전체' || plan.char === currentFilter) {
            ...
            if (!isNaN(targetLevelNum)) {
                ...
                if (!pred.error) {
                    if (!pred.hasRealLogs) {
                        predictionHtml = `
                            <div class="exp-predict-badge" style="background: #f8fafc; border-color: #e2e8f0; color: #64748b;">
    ...
    const uniqueChars = [...new Set(characters.map(c => typeof c === 'string' ? c : (c ? (c.name || c.characterName || c.character_name || c.id) : '')))].filter(Boolean);

    uniqueChars.forEach(name => { 
        const charObj = characters.find(c => (typeof c === 'string' ? c : (c.name || c.characterName || c.character_name || c.id)) === name) || {};
        ...
        // 💡 [서버명 자동 교차 검색 Engine]: OMNI V15 캐시 및 캐릭터 데이터에서 정확한 서버명을 조회합니다.
        const serverName = window.getMapleServerName(charObj) || window.getMapleServerName(name) || '서버미정';
        
        selectOptions += `<option value="${name}" ${currentFilter === name ? 'selected' : ''}>👤 ${name} (${job} / Lv.${lv} / ${serverName})</option>`; 
    });

    // 선택된 캐릭터의 서버 아이콘(icon/world/서버명.png) 이미지 바인딩
    const activeCharObj = characters.find(c => (typeof c === 'string' ? c : (c.name || c.characterName || c.character_name || c.id)) === currentFilter) || {};
    const activeServerName = window.getMapleServerName(activeCharObj) || window.getMapleServerName(currentFilter);
    ...
    if (currentFilter !== '전체') {
        const charObj = characters.find(c => (typeof c === 'string' ? c : (c.name || c.characterName || c.character_name || c.id)) === currentFilter) || {};
        ...
        const sideServer = window.getMapleServerName(charObj) || window.getMapleServerName(currentFilter);
        ...
        levelUpSideWidgetHtml = `
            <div class="omni-mini-calendar-widget" style="margin-top: 14px; text-align: left;">
                ...
                <div style="display: flex; flex-direction: column; gap: 8px; font-size: 11.5px; padding-top: 4px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; background: var(--omni-bg-clean, #f8fafc); padding: 8px 10px; border-radius: 8px; border: 1px dashed var(--omni-card-border-line, #e2e8f0);">
                        <span style="font-weight: 700; color: var(--omni-text-sub);">현재 전투원 레벨</span>
                        <strong style="font-weight: 800; color: var(--omni-primary-accent, #6366f1);">Lv.${curLv}</strong>
                    </div>
                    <div style="display: flex; justify-content: space-between; align-items: center; background: var(--omni-bg-clean, #f8fafc); padding: 8px 10px; border-radius: 8px; border: 1px dashed var(--omni-card-border-line, #e2e8f0);">
                        <span style="font-weight: 700; color: var(--omni-text-sub);">목표 레벨 (Lv.${goalLv})</span>
                        <strong style="font-weight: 800; color: var(--omni-primary-accent, #6366f1);">${hasLogs ? `앞으로 ${predDays}일 뒤` : '[데이터가 없습니다.]'}</strong>
                    </div>
                    <div style="font-size: 11px; font-weight: 700; color: var(--omni-text-main); margin-top: 4px; padding: 6px 4px; border-top: 1px dashed var(--omni-card-border-line, #e2e8f0);">
        ...
        levelUpSideWidgetHtml = `
            <div class="omni-mini-calendar-widget" style="margin-top: 14px; text-align: left;">
                ...
                <div style="font-size: 11.5px; color: var(--omni-text-sub); padding: 8px 4px;">
    ...
    let quickStatsWidgetHtml = `
        <div class="omni-mini-calendar-widget" style="margin-top: 14px; text-align: left;">
            ...
            <div style="display: flex; flex-direction: column; gap: 6px; font-size: 11.5px; padding-top: 4px; color: var(--omni-text-sub);">
                <div style="display: flex; justify-content: space-between;">
                    <span>• 등록된 전략 계획서</span>
                    <strong style="color: var(--omni-text-main);">${planList.length}건</strong>
                </div>
                <div style="display: flex; justify-content: space-between;">
                    <span>• 연동된 캐릭터 수</span>
                    <strong style="color: var(--omni-text-main);">${uniqueChars.length}명</strong>
                </div>
                <div style="font-size: 10.5px; color: #94a3b8; border-top: 1px dashed var(--omni-card-border-line, #e2e8f0); padding-top: 6px; margin-top: 2px;">
                    💡 [사냥 기록지랑 연동되어 추산됩니다]
                </div>
            </div>
        </div>
    ...
    const emptyStateHtml = `
        <div class="omni-empty-state-card">
            <div class="empty-icon">📌</div>
            <div class="empty-title">수립된 전략 계획서가 존재하지 않습니다</div>
            <p class="empty-desc">상단 입력창에서 캐릭터 선택, 목표 레벨, 육성 동선을 설정 후 <strong>[계획 수립]</strong> 버튼을 눌러보세요.</p>
        </div>
    `;
        ...
        <div class="omni-planner-container">
            ...
            <div class="omni-planner-dashboard-layout">
                <div class="planner-left-stack">
                    ...
                    <div class="planner-grid-cards">${cardsHtml ? cardsHtml : emptyStateHtml}</div>
                </div>
...
window.buildMiniCalendarComponentMarkup = function(planCount) {
    ...
    weekLabels.forEach(l => { 
        daysHtml += `<div class="calendar-day-label" style="font-size: 9px; font-weight: 800; color: var(--omni-text-sub); padding: 2px 0;">${l}</div>`; 
    });
    daysHtml += `<div class="calendar-date-cell empty-cell"></div>`;

    for(let d=1; d<=30; d++) {
        const isChecked = window.omniTodoState && window.omniTodoState.calendarCheckedDays && window.omniTodoState.calendarCheckedDays[d] === true;
        daysHtml += `<div class="calendar-date-cell ${isChecked ? 'is-checked' : ''}" onclick="window.renderDayHourlyExpChart(${d})" title="7월 ${d}일 시간별 경험치 보기" style="padding: 3px 0; font-size: 10px; font-weight: 700; border-radius: 6px; cursor: pointer; height: 26px; display: flex; align-items: center; justify-content: center; margin: 0 auto; width: 100%; max-width: 32px;">${d}</div>`;
    }
    return `
        <div class="omni-mini-calendar-widget" style="padding: 14px; gap: 8px; border-radius: 16px; box-sizing: border-box; width: 100%;">
            <div class="calendar-header-title" style="font-size: 11px; padding-bottom: 6px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px dashed var(--omni-card-border-line, #cbd5e1);">
                <span style="font-weight: 900; color: var(--omni-text-main);">🗓️ 성장 스케줄러 달력</span>
                <span style="font-size: 8.5px; font-weight: 800; color: var(--omni-primary-accent, #6366f1); background: var(--omni-bg-clean, #f8fafc); padding: 2px 5px; border-radius: 4px; border: 1px dashed var(--omni-card-border-line, #cbd5e1);">날짜클릭 ➡️ EXP</span>
            </div>
            <div class="calendar-days-grid" style="gap: 3px; grid-template-columns: repeat(7, 1fr); text-align: center; justify-items: center;">${daysHtml}</div>
        </div>
    `;
};
...
window.renderDayHourlyExpChart = function(dayNum) {
    ...
    const panelTitleEl = document.querySelector('.chart-panel-title');
    if (panelTitleEl) {
        panelTitleEl.innerHTML = `📊 [${targetChar}] ${dayNum}일 시간대별 경험치 획득 상세 추이 (억 EXP) <span style="font-size:10px; color:#6366f1; font-weight:bold;">[사냥 기록지랑 연동되어 추산됩니다]</span>`;
    }
...
window.toggleCalendarDateCell = function(dayNum) {
    if (!window.omniTodoState) window.omniTodoState = {};
    if (!window.omniTodoState.calendarCheckedDays) window.omniTodoState.calendarCheckedDays = {};
