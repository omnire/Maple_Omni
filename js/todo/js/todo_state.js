 ...
 * 역할: 투두 시스템의 데이터 전역 관리, 공통 UI 컴포넌트 공급 및 메뉴 위치 강제 보정
...
/**
 * 📡 넥슨 OpenAPI 통신 수급 완료 시 비동기 파싱 데이터를 투두 영구 원장 메모리에 즉각 강제 매핑 및 결착하는 연동 허브입니다.
 */
window.syncTodoCharacterOnSearch = function(charName, basicInfo, apiHomework) {
    ...
    if (dailyArr.length === 0 && weeklyArr.length === 0 && bossArr.length === 0) {
        console.warn(`[OMNI LINK V14] "${charName}" 스케줄러 데이터가 완전히 비어 있습니다. 이 캐릭터가 2026-06-25 이후 게임에 접속한 이력이 없거나, API 키에 '스케줄러 정보 조회' 권한이 없을 수 있습니다.`);
    }
    ...
    const syncBossKey = (keyword, diff, objectKey) => {
        const target = bossArr.find(c => c.content_name && c.content_name.includes(keyword) && omniMatchesDifficulty(c.difficulty, diff));
